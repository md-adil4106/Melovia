  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed inset-y-0 right-0 z-50 w-full max-w-xl bg-[#0e1218] border-l border-[#232a3b] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-right duration-300"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1d2331]">
          <div className="flex items-center gap-2.5">
            <ListMusic className="w-5 h-5 text-[#d4af37]" />
            <h2 className="text-base font-bold text-[#f1f3f7] font-serif-display">
              Playlist Builder
            </h2>
          </div>
          <button
            ...
            className="p-1.5 rounded-lg text-[#8c96a8] hover:text-[#f1f3f7] hover:bg-[#1b2230] transition-colors focus:outline-none focus:ring-2 focus:ring-[#d4af37]"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Controls */}
        <div className="px-5 py-4 border-b border-[#1d2331] space-y-4">
          {/* Arc Preset Buttons */}
          <div>
            <label className="text-xs font-semibold text-[#8c96a8] uppercase tracking-wider mb-2 block">
              Energy Arc
            </label>
              ...
              {(Object.keys(ARC_INFO) as ArcPreset[]).map((arc) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={active}
                    className={`flex-1 flex flex-col items-center gap-1 px-2 py-2 rounded-lg text-xs font-medium transition-all focus:outline-none focus:ring-2 focus:ring-[#d4af37] ${
                      active
                        ? "bg-[#d4af37]/15 border border-[#d4af37]/60 text-[#d4af37]"
                        : "bg-[#141923] border border-[#232a3b] text-[#8c96a8] hover:border-[#38435d] hover:text-[#c8d0de]"
                    }`}
                  >
          ...
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-[#8c96a8] uppercase tracking-wider">
              Playlist Length
            </label>
            <div className="flex items-center gap-2">
              <button
                ...
                className="p-1 rounded bg-[#1b2230] border border-[#2b354a] text-[#8c96a8] hover:text-[#f1f3f7] disabled:opacity-40 transition-colors focus:outline-none focus:ring-1 focus:ring-[#d4af37]"
              >
                <Minus className="w-3.5 h-3.5" />
              </button>
              <span className="font-mono text-sm font-bold text-[#f1f3f7] w-8 text-center">
                {playlistLength}
              </span>
              <button
                type="button"
              ...
              <button
                ...
                className="ml-2 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-gradient-to-r from-[#d4af37] to-[#b38e24] text-black transition-all active:scale-95 disabled:opacity-40 focus:outline-none focus:ring-2 focus:ring-[#d4af37]"
              >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          {/* Loading */}
          {sequenceMutation.isPending && (
            <div className="flex items-center justify-center py-12 text-[#8c96a8]">
              <RefreshCw className="w-6 h-6 animate-spin mr-3 text-[#d4af37]" />
          ...
          {sequenceMutation.isError && (
            <div className="bg-rose-950/40 border border-rose-800/60 rounded-xl p-4 text-rose-200 text-sm">
            ...
            <>
              ...
              {arcPoints.length > 1 && (
                <div>
                  ...
                  <div className="bg-[#141923] border border-[#232a3b] rounded-xl p-3">
                    <svg
                    ...
                    >
                      ...
                      {[0, 0.5, 1].map((e) => (
                        <text
                          ...
                          y={toY(e) + 3}
                          textAnchor="end"
                          className="fill-[#5a667d] text-[9px]"
                        >
                          {e.toFixed(1)}
                        </text>
                      ))}
                      {/* Target arc (dashed gold) */}
                      {targetPath && (
                        <path
                          d={targetPath}
                          fill="none"
