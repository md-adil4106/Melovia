"""Recommendations API router for generating relevance-ranked recommendations."""
...
@router.post(
    ...
    description=(
        ...
        "candidates, and computes relevance scores."
    ),
)
async def create_recommendations(
    ...
    top_items = scored_list.top_n(payload.n)
    ...
    for item in top_items:
        ...
        response_items.append(
            RecommendedTrackItem(
                track=track_resp,
                score=item.score,
                signals=item.signals if payload.include_signals else None,
            )
        )

    return RecommendationResponse(
        candidate_set_id=candidate_set_id,
        total_candidates=scored_list.total_candidates,
